...
tx_hash = contractADMM.functions.newUser(EV1.userType).transact()
tx_receipt = web3.eth.waitForTransactionReceipt(tx_hash)
gasSpend += tx_receipt.gasUsed

# preferences
EV1.setPreferences(1/3,1/3,1/3)  # (costs, exchanges with community, comfort)
...
SOCinit = 20/100
SOCrequired = 50/100
arrivalTimeEV1 = pd.to_datetime('2019-06-10 09:15')
departureTimeEV1 = pd.to_datetime('2019-06-10 12:30')

EV1.setChargingTime(arrivalTimeEV1,departureTimeEV1)
EV1.setParameters(PmaxEV,EmaxEV,etaEV,SOCmin,SOCmax,SOCinit)
EV1.setSOCrequired(SOCrequired)
...
while result ==0: # while stopping criteria of global optimization not reached
    ...
    EV1.optimization(startTime, endTime, timeStep, price_imp, price_PV, price_B, rho, L, yPVforecast)   
    ...
    X_historic.append(EV1.optimizedProfile)
    L_historic.append(L)
...
#    # save data
    ...
    EV1.CalculateSOC(timestep)
    SOC_EV1 = EV1.SOC
    
    X_EV1 = EV1.optimizedProfile
    X_historic_EV1 = X_historic
    gasSpend_EV1 = gasSpend
    gasSpend_without_initialization_EV1 = gasSpend - gasSpend_initialization
    etherSpend_EV1 = etherSpend - web3.eth.getBalance(web3.eth.defaultAccount)
    
    
    # save variables
    filename = 'Results_EV1'
    with open(filename, 'wb') as f:
        pickle.dump(X_historic_EV1,f)
        pickle.dump(SOC_EV1,f)
        pickle.dump(gasSpend_EV1,f)
        pickle.dump(gasSpend_without_initialization_EV1,f)
        pickle.dump(etherSpend_EV1,f)
