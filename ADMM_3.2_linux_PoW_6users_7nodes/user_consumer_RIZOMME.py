...
forecast_consumer_RIZOMME =  pd.read_csv(fnamePV,sep=';',names = ['timeVector','power'],index_col=0,squeeze=True,parse_dates=True)
forecast_consumer_RIZOMME = forecast_consumer_RIZOMME.asfreq(freq =str(timeStep)+'T') # retime
...
while result ==0: # while stopping criteria of global optimization not reached
    ...
    #save date
    X_consumer_RIZOMME = Building.optimizedProfile
    X_historic_consumer_RIZOMME = X_historic
    gasSpend_consumer_RIZOMME = gasSpend
    gasSpend_without_initialization_consumer_RIZOMME = gasSpend - gasSpend_initialization
    etherSpend_consumer_RIZOMME = etherSpend - web3.eth.getBalance(web3.eth.defaultAccount)
    #forecast_consumer_RIZOMME = pd.Series(np.array(forecast_consumer_RIZOMME)/10**6,timeVector)
    forecast_consumer_RIZOMME = np.array(forecast_consumer_RIZOMME)/10**6
    
    
    # save variable X_historic
    filename = 'Results_consumer_RIZOMME'
    with open(filename, 'wb') as f:
        pickle.dump(X_historic_consumer_RIZOMME,f)
        pickle.dump(forecast_consumer_RIZOMME,f)
        pickle.dump(gasSpend_consumer_RIZOMME,f)
        pickle.dump(gasSpend_without_initialization_consumer_RIZOMME,f)
        pickle.dump(etherSpend_consumer_RIZOMME,f)
