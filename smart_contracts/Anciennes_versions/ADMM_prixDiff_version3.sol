...
// GLOBAL OPTIMIZATION
    ...
    function updateL() public onlyOwner {
        ...
        for (uint i = 0; i<4*nbSteps; i++) {
            ...
            if (iterations==0){
                if(getArraySum(sum_X_global, nbSteps, 2*nbSteps)>0) { //sum(q_i)>0 <=> PVimported > PVproduction ==> no PV curtailment
                    if (L[i] < 0){
                        L_PV[i] = 0;
                        L_2[i] = L[i];
                    }
                ...
                else{//sum(q_i)>0 <=> PVproduction>PVimported  ==> no diminution of PV import
                    if(L[i]>0){
                        L_PV[i] = L[i];
                        L_2[i] = 0;
                    }
	...
	function getOptimizationParameters_PV() public view returns (int[] memory, int){
	    return (L_PV,rho);
	}
	
	function getOptimizationParameters_2() public view returns (int[] memory, int){
	    return (L_2,rho);
	}
