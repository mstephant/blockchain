...
// ----------------------------------------------------------------------------------	
	...
	function updateZ() public onlyOwner {
	    for (uint i = 0; i<nbSteps; i++) {
	        meanX[i] = X_global[i]/int(N);
	         //
	        if (iterations ==0) { // first iteration
	            Z[i] = X_global[i]/int(N);
	        }
	        Z_previous[i] = Z[i];
    ...
    function updateURS() public onlyOwner {
	    for (uint i = 0; i<nbSteps; i++) {
			U[i] = U[i] + meanX[i]- Z[i];
			R[i] = meanX[i] - Z[i];
	...
	function updateRho() public onlyOwner {
	    if (normR>mu**2 * normS){
	        rho = tau*rho;
	        AAA = 1;
	    }
	...
	function stopCriterion() public onlyOwner {   
	    // stop criterion 
