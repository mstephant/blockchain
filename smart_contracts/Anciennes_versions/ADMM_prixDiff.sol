...
contract ADMM {
    ...
    uint coeffPriceLocal; // coeff price local 
...
// ----------------------------------------------------------------------------------
	function setTimeInterval(uint _startTime, uint _endTime, uint _timeStep) public onlyOwner {
		...
		for (uint i=0; i < 3*nbSteps; i++){ //3*nbSteps because 3 variables (p,q,lamda)
	       sum_X_global.push(0);
	       meanX.push(0);
	       Z.push(0);
	       Z_previous.push(0);
...
// ----------------------------------------------------------------------------------	
	...
	function setPriceLocal (uint  _coeffPriceLocal) public onlyOwner {
	    // price_local = coeffPriceLocal* price_imp
	    coeffPriceLocal = _coeffPriceLocal;
    ...
    function getPriceLocal() public view returns (uint){
        return (coeffPriceLocal);
    }
	
// ----------------------------------------------------------------------------------
		
	function sendOptimizedProfile(int[] memory _optimProfile) public {
	    require(_optimProfile.length == 3*nbSteps, "vector doesn't have good size");
	...
	function updateZ() public onlyOwner {
	    ...
	    for (uint i=0; i<nbSteps; i++) {
	        if (sum_X_global[2*nbSteps+i]>Pmax){
	            ...
	            //Z[i] = Pmax/N - 0.3;
	            Z[nbSteps+i] = 0;
	            Z[2*nbSteps+i] = Pmax/int(N) -3*10**(5);
	            //Z[i] = Pmax/N - 0.3;
	        }
	        else {
	            Z[i] = U[i] + meanX[i];
	            Z[nbSteps+i] = 0;
	            Z[2*nbSteps+i] = -(U[i] + meanX[i]);
	        }
	    }
	}


    function updateURSLrho() public onlyOwner {
        // update U, R, S, L and rho
	    for (uint i = 0; i<3*nbSteps; i++) {
			U[i] = U[i] + meanX[i]- Z[i];
			R[i] = meanX[i] - Z[i];
