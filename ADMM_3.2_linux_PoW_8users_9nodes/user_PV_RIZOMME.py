...
script for PV generator profile optimization 
...
tx_hash = contractADMM.functions.newUser(PV1.userType).transact()
tx_receipt = web3.eth.waitForTransactionReceipt(tx_hash)
gasSpend += tx_receipt.gasUsed

# preferences
PV1.setPreferences(1/3,1/3,1/3)  #(costs, exchanges,comfort)
...
forecast_PV_RIZOMME =  pd.read_csv(fnamePV,sep=';',names = ['timeVector','power'],index_col=0,squeeze=True,parse_dates=True)
forecast_PV_RIZOMME = forecast_PV_RIZOMME.asfreq(freq =str(timeStep)+'T') # retime
forecast_PV_RIZOMME = forecast_PV_RIZOMME.interpolate()
forecast_PV_RIZOMME = forecast_PV_RIZOMME[startTime:endTime] # negative values!
...
while result ==0: # while stopping criteria of global optimization not reached
    ...
    L = L/10**6
    rho = rho/10**6
    if curtailPV==0:  # sum(q_i)>0 <=> importsPV > productionPV ==> no PV curtailement, diminution of PV imports from consumers
    	L[length:2*length] = np.maximum(L[length:2*length],0)

    time.sleep(15)###
    # optimization of profile
    PV1.optimization(startTime, endTime, timeStep, price_imp, price_PV, rho, L)
    profileToSend = tl.toListInt(PV1.optimizedProfile)
    ...
    X_historic.append(PV1.optimizedProfile)
    L_historic.append(L)
    ...
    # save data
    X_PV_RIZOMME = PV1.optimizedProfile
    X_historic_PV_RIZOMME = X_historic
    gasSpend_PV_RIZOMME = gasSpend
    gasSpend_without_initialization_PV_RIZOMME = gasSpend - gasSpend_initialization
    etherSpend_PV_RIZOMME = etherSpend - web3.eth.getBalance(web3.eth.defaultAccount)
    #forecast_PV_RIZOMME = pd.Series(np.array(forecast_PV_RIZOMME)/10**6,timeVector)
    forecast_PV_RIZOMME = np.array(forecast_PV_RIZOMME)/10**6
    
    # save variable X_historic
    filename = 'Results_PV_RIZOMME'
    with open(filename, 'wb') as f:
        pickle.dump(X_historic_PV_RIZOMME,f)
        pickle.dump(forecast_PV_RIZOMME,f)
        pickle.dump(gasSpend_PV_RIZOMME,f)
        pickle.dump(gasSpend_without_initialization_PV_RIZOMME,f)
        pickle.dump(etherSpend_PV_RIZOMME,f)
