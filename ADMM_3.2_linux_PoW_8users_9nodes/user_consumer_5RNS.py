...
# get forecast_consumer_5RNS profile
fnamePV ='../Data_' + month + '19/conso_5RNS_' + month + '19.csv'
forecast_consumer_5RNS =  pd.read_csv(fnamePV,sep=';',names = ['timeVector','power'],index_col=0,squeeze=True,parse_dates=True)
forecast_consumer_5RNS = forecast_consumer_5RNS.asfreq(freq =str(timeStep)+'T') # retime
forecast_consumer_5RNS = forecast_consumer_5RNS.interpolate()
forecast_consumer_5RNS = forecast_consumer_5RNS[startTime:endTime]
Building.setForecast(forecast_consumer_5RNS)
forecast_consumer_5RNS = tl.toListInt(forecast_consumer_5RNS)
...
while result ==0: # while stopping criteria of global optimization not reached
    ...
    #save date
    X_consumer_5RNS = Building.optimizedProfile
    X_historic_consumer_5RNS = X_historic
    gasSpend_consumer_5RNS = gasSpend
    gasSpend_without_initialization_consumer_5RNS = gasSpend - gasSpend_initialization
    etherSpend_consumer_5RNS = etherSpend - web3.eth.getBalance(web3.eth.defaultAccount)
    #forecast_consumer_5RNS = pd.Series(np.array(forecast_consumer_5RNS)/10**6,timeVector)
    forecast_consumer_5RNS = np.array(forecast_consumer_5RNS)/10**6
    
    
    # save variable X_historic
    filename = 'Results_consumer_5RNS'
    with open(filename, 'wb') as f:
        pickle.dump(X_historic_consumer_5RNS,f)
        pickle.dump(forecast_consumer_5RNS,f)
        pickle.dump(gasSpend_consumer_5RNS,f)
        pickle.dump(gasSpend_without_initialization_consumer_5RNS,f)
        pickle.dump(etherSpend_consumer_5RNS,f)
