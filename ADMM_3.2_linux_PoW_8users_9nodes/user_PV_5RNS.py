...
forecast_PV_5RNS =  pd.read_csv(fnamePV,sep=';',names = ['timeVector','power'],index_col=0,squeeze=True,parse_dates=True)
...
while result ==0: # while stopping criteria of global optimization not reached
    ...
    # save data
    X = PV1.optimizedProfile
    X_PV_5RNS = -X
    X_historic_PV_5RNS = X_historic
    gasSpend_PV_5RNS = gasSpend
    gasSpend_without_initialization_PV_5RNS = gasSpend - gasSpend_initialization
    etherSpend_PV_5RNS = etherSpend - web3.eth.getBalance(web3.eth.defaultAccount)
    #forecast_PV_5RNS = pd.Series(np.array(forecast_PV_5RNS)/10**6,timeVector)
    forecast_PV_5RNS = np.array(forecast_PV_5RNS)/10**6
    #
    #
    #
    ...
    with open(filename, 'wb') as f:
        pickle.dump(X_historic_PV_5RNS,f)
        pickle.dump(forecast_PV_5RNS,f)
        pickle.dump(gasSpend_PV_5RNS,f)
        pickle.dump(gasSpend_without_initialization_PV_5RNS,f)
        pickle.dump(etherSpend_PV_5RNS,f)
